...
st.markdown("""It is **highly recommended** to read the [usage instructions and imaging recommendations](https://github.com/diyavakil/counterplusHITL/blob/main/README.md) before use to ensure the best results. Still having issues, or want to submit suggestions for improvements? [Submit a support ticket](https://github.com/diyavakil/counterplushitl/issues/new) or [send me an email with your query](mailto:dvakil@ucmerced.edu).""")
...
# upload img
uploaded_file = st.file_uploader("Upload an image", type=["png", "heic", "jpeg", "jpg", "bmp", "webp", "tiff", "tif", "dng", "avif", "jp2", "mpo", "pfm"], help="Supported formats: PNG, HEIC, JPEG/JPG, BMP, WEBP, TIFF/TIF, DNG, AVIF, JP2, MPO, PFM")
if uploaded_file is not None:
    try:
        ...
        with st.expander("YOLO Detection Options", expanded=True): # expander? i hardly know her
            conf_threshold = st.slider("Confidence threshold", min_value=0.0, max_value=1.0, value=0.0, step=0.01, help="Filter out detections below this confidence level. It is not recommended to adjust this value unless you have a large number of false positive detections. Otherwise, keep it at 0.00.")
            show_conf = st.checkbox("Show confidence values", value=False, help="Display confidence scores for each detected colony. It is recommended to keep this setting off to make it easier to perform the manual corrections.")
