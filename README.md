...
Quantify bacterial colonies with automated YOLO-based object detection and manual human-in-the-loop corrections. (Please read this before asking why it didn't work)

**NOTE:** The current version (v1) tends to have a decent number of false negative detections. The human-in-the-loop addition is meant to adjust the automatic count to ensure accuracy. All images are highly recommended to be checked post-inference manually to make any necessary corrections.
...
1. **Upload your image**  
   Upload the Petri dish image you want to analyze (`.jpg`, `.jpeg`, or `.png`). Once uploaded, your original image will be displayed. (If on mobile, you can also take a picture in the app)
...
These are just suggestions, but the model's ability to efficiently run inference does greatly depend on the quality of the input image. Garbage input = garbage inference
